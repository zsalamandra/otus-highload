...
def main():
    global friend_token
    print(f"{BLUE}=== Демонстрация WebSocket обновления ленты ==={NC}")

    # Хост сервиса - используем имя контейнера из docker-compose
    host = os.environ.get("TARGET_HOST", "z-social-network-hw6-app-1")
    ...
    try:
        login_response = requests.post(f"http://{host}:8085/login", json=login_data_friend)
    ...
    # Настройка websocket с передачей userId в URL
    websocket.enableTrace(True)
    ws_url = f"ws://{host}:8085/ws/websocket"
    ...
    try:
        login_response = requests.post(f"http://{host}:8085/login", json=login_data_creator)
    ...
    try:
        ...
        add_friend_response = requests.post(
            f"http://{host}:8085/friend/add?friendId={friend_id}",
    ...
    post_content = f"Демонстрационный пост для проверки WebSocket {time.ctime()}"
    ...
    try:
        create_response = requests.post(f"http://{host}:8085/post/create",
                                        json=post_data,
                                        headers=headers)
