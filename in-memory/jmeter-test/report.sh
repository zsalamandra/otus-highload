...
show_stats_table() {
  ...
  # Сохранение результатов в файл для будущего использования
  # Если в параметрах указана переменная STORAGE_TYPE
  if [ -n "$STORAGE_TYPE" ]; then
    echo ""
    echo "Сохранение результатов тестирования для типа хранилища: $STORAGE_TYPE"
    jq '{
      "Send Message": {
    ...
    }' $STATS_FILE > "/results/${STORAGE_TYPE}_results.json"
  fi
}
